...
const Register = () => {
  const [form, setForm] = useState({ name: '', email: '', password: '', college: '', cgpa: '' });
      ...
      >
        ...
        <div>
          ...
          <h1 className="font-display font-bold text-4xl tracking-tight leading-tight mb-4">
            Your profile, scored against every JD you target.
          </h1>
          <p className="font-body text-sm mb-10" style={{ color: '#71717A' }}>
            Analyze unlimited job descriptions, close skill gaps, generate tailored resumes.
          </p>
        ...
        <div className="w-full max-w-sm p-1.5 rounded-[2rem]" style={{ backgroundColor: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.06)' }}>
          <form onSubmit={handleSubmit} className="rounded-[calc(2rem-0.375rem)] p-8 flex flex-col gap-4" style={{ backgroundColor: '#0A0A0A', boxShadow: 'inset 0 1px 1px rgba(255,255,255,0.04)' }}>
            ...
            <div className="flex flex-col gap-1">
              <label className="font-body text-xs" style={{ color: '#71717A' }}>Full name</label>
              <input name="name" value={form.name} onChange={handleChange} className={inputClass} style={inputStyle} {...focusHandlers} required />
            </div>

            <div className="flex flex-col gap-1">
              <label className="font-body text-xs" style={{ color: '#71717A' }}>Email</label>
              <input type="email" name="email" value={form.email} onChange={handleChange} className={inputClass} style={inputStyle} {...focusHandlers} required />
            </div>

            <div className="flex flex-col gap-1">
              <label className="font-body text-xs" style={{ color: '#71717A' }}>Password</label>
              <input type="password" name="password" value={form.password} onChange={handleChange} className={inputClass} style={inputStyle} {...focusHandlers} required />
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div className="flex flex-col gap-1">
                <label className="font-body text-xs" style={{ color: '#71717A' }}>College</label>
                <input name="college" value={form.college} onChange={handleChange} className={inputClass} style={inputStyle} {...focusHandlers} />
              </div>
              <div className="flex flex-col gap-1">
                <label className="font-body text-xs" style={{ color: '#71717A' }}>CGPA</label>
                <input type="number" step="0.01" name="cgpa" value={form.cgpa} onChange={handleChange} className={inputClass} style={inputStyle} {...focusHandlers} />
              </div>
            </div>
            ...
            <p className="text-sm font-body text-center mt-2" style={{ color: '#71717A' }}>
              Already have an account? <Link to="/login" style={{ color: '#F59E0B' }} className="font-medium">Sign in</Link>
            </p>
          </form>
        </div>
      </motion.div>
